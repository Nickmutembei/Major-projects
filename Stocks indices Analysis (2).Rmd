...
for (i in 1:(length(stocks)-1)) {
  for (j in (i+1):length(stocks)) {
    ...
    adf_result <- adf.test(as.numeric(stock_prices[, 1] - stock_prices[, 2]), alternative = "stationary")
    p_value <- adf_result$p.value
    if (p_value < 0.05) {
      cat("Found cointegrated pair:", stocks[i], "and", stocks[j], "\n")
      trading_pairs[[paste(stocks[i], stocks[j], sep = "_")]] <- stock_prices
    }
  }
}
...
trading_strategy <- function(pair_prices) {
  ...
  # Create a data frame with the trading signals
  signals <- data.frame(Buy = buy_signal, Sell = sell_signal)
  
  # Combine the trading signals with the original price data
  trades <- cbind(pair_prices, signals)
  
  # Initialize the trading position and the portfolio value
  position <- 0
  portfolio_value <- 0
  ...
  for (i in 1:nrow(trades)) {
    ...
    if (trades[i, "Buy"]) {
      # Buy the first stock and sell the second stock
      position <- 1
      portfolio_value <- portfolio_value - trades[i, 1] + trades[i, 2]
    }
    # Check if we have a sell signal
    else if (trades[i, "Sell"]) {
      # Sell the first stock and buy the second stock
      position <- -1
      portfolio_value <- portfolio_value + trades[i, 1] - trades[i, 2]
    }
    # Otherwise, hold the position
    else {
      portfolio_value <- portfolio_value + position * (trades[i, 1] - trades[i, 2])
    }
  }
...
# Loop over the trading pairs
for (pair_name in names(trading_pairs)) {
  # Get the price data for the pair
  pair_prices <- trading_pairs[[pair_name]]
  
  # Run the trading strategy on the pair
  pair_value <- trading_strategy(pair_prices)
  
  # Print the result
  cat("Pair", pair_name, "generated a profit of", pair_value, "\n")
}
